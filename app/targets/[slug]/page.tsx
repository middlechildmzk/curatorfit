    ...
    <main className="mx-auto max-w-5xl px-5 py-12">
      ...
      <section className="card overflow-hidden">
        ...
        <div className="grid gap-6 p-6 md:grid-cols-[1fr_320px] md:p-8">
          <div>
            <h2 className="text-2xl font-black">Why this target may fit</h2>
            <p className="mt-3 leading-7 text-slate-600">{target.fitNotes}</p>
            {target.recommendedPitchAngle ? <><h2 className="mt-8 text-2xl font-black">Recommended pitch angle</h2><p className="mt-3 leading-7 text-slate-600">{target.recommendedPitchAngle}</p></> : null}
            {target.samplePitchNote ? <><h2 className="mt-8 text-2xl font-black">Sample pitch note</h2><p className="mt-3 rounded-3xl border border-line bg-slate-50 p-5 leading-7 text-slate-700">{target.samplePitchNote}</p></> : null}
            ...
            {target.aiMusicNotes ? <><h2 className="mt-8 text-2xl font-black">AI music policy notes</h2><p className="mt-3 flex gap-3 leading-7 text-slate-600"><Sparkles className="mt-1 h-5 w-5 shrink-0 text-brand" /> {target.aiMusicNotes}</p></> : null}
          </div>
          <aside className="rounded-3xl border border-line bg-slate-50 p-5">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              {target.subgenres?.slice(0, 4).map((genre) => <span key={genre} className="pill">{genre}</span>)}
              {target.energyLevel ? <span className="pill">{target.energyLevel} energy</span> : null}
              {target.allowsAiAssistedMusic ? <span className="pill">AI: {target.allowsAiAssistedMusic.replaceAll('_', ' ')}</span> : null}
            </div>
            <div className="mt-5 grid gap-3 text-sm text-slate-600">
              ...
              {target.freeOrPaid ? <div className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 text-brand2" /> Cost basis: {target.freeOrPaid}</div> : null}
              {target.responseExpectation ? <div className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 text-brand2" /> Response: {target.responseExpectation}</div> : null}
            </div>
